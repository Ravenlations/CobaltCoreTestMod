...
using Ravenlations.Raven.Artifacts;
using Microsoft.Extensions.Logging;
using Nanoray.PluginManager;
using System;
using System.Collections.Generic;
using System.Linq;
using Ravenlations.Raven.Artifacts.Ship;
 ...
 * Notice the ': SimpleMod'. This means ModEntry is a sublass (child) of the superclass SimpleMod (parent) from Nickel. This will help us use Nickel's functions more easily! */
public sealed class ModEntry : SimpleMod
{
    ...
    internal static IReadOnlyList<Type> DemoCharacter_CommonCard_Types { get; } = [
        ...
        typeof(HackerBoost),
        typeof(Missclick),
        typeof(FortniteShield),
        typeof(FortniteDance),
        typeof(PopUp),
    ...
    internal static IReadOnlyList<Type> DemoCharacter_Rare_Types { get; } = [
        typeof(SkillIssue),
        typeof(PotOfGreed),
        typeof(FirmwareUpdate),
        typeof(GodMode),
    ...
    public ModEntry(IPluginPackage<IModManifest> package, IModHelper helper, ILogger logger) : base(package, helper, logger)
    {
        ...
        RavenDeck = helper.Content.Decks.RegisterDeck("DemoDeck", new DeckConfiguration()
        {
        ...
        * It's totally fine to assign them if you'd like, but we don't have a reason to so in this mod */
        ...
        foreach (var cardType in DemoMod_AllCard_Types)
            AccessTools.DeclaredMethod(cardType, nameof(RavenCard.Register))?.Invoke(null, [package , helper]);
        ...
        var demoShipPartWing        = helper.Content.Ships.RegisterPart("DemoPart.Wing", new PartConfiguration(){Sprite = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/ships/demowing.png")).Sprite});
        var demoShipPartMissiles    = helper.Content.Ships.RegisterPart("DemoPart.Missiles", new PartConfiguration() { Sprite = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/ships/demomissiles.png")).Sprite });
        var demoShipPartCannon      = helper.Content.Ships.RegisterPart("DemoPart.Cannon", new PartConfiguration(){Sprite = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/ships/democannon.png")).Sprite});
        var demoShipPartCockpit     = helper.Content.Ships.RegisterPart("DemoPart.Cockpit", new PartConfiguration(){Sprite = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/ships/democockpit.png")).Sprite});
        ...
        BackwardsMissilesPermanent = helper.Content.Statuses.RegisterStatus("BackwardsMissilesPermanent", new()
        {
            Definition = new()
            {
                icon = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/icons/PermanentMissiles.png")).Sprite,
                color = new("f05050"),
                isGood = true
            },
            Name = AnyLocalizations.Bind(["status", "BackwardsMissiles", "name"]).Localize,
            Description = AnyLocalizations.Bind(["status", "BackwardsMissiles", "description"]).Localize
        });
        _ = new BackwardsMissilesPermanentManager();
